...
iris %>%
  ...
  count(Petal.Length >= 1.462)
iris %>%
  select(Species, Petal.Length) %>%
  filter(Species == "versicolor") %>%
  count(Petal.Length >= 4.260
  )
iris %>%
  select(Species, Petal.Length) %>%
  filter(Species == "virginica") %>%
  count(Petal.Length >= 5.552
  )
...
#you lose a level because its only adding the number of values in the first column. Putting ht first means there's an
#NA value that is not counted. 
...
flights %>%
  ...
  count(dest) %>%
  arrange(desc(n))
#O'hare has the most arrivals with 17283
...
tailnum_plane <- planes %>%
  select(tailnum) %>%
  arrange(desc(tailnum))

flight_tailnum <- left_join(tailnum_plane, flight_2013)
flight_tailnum %>%
  count(tailnum) %>% 
  arrange(desc(n))
